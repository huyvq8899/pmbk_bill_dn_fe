...
import { getHeightBangKe, getHeightBangKeKhongChiTiet6, getListEmptyBangKe, setStyleTooltipError } from 'src/app/shared/SharedFunction';
...
@Component({
  selector: 'app-list-hoa-don-xoa-bo-modal',
  templateUrl: './list-hoa-don-xoa-bo-modal.component.html',
  styleUrls: ['./list-hoa-don-xoa-bo-modal.component.scss']
})
export class ListHoaDonXoaBoModalComponent extends TabShortKeyEventHandler implements OnInit, AfterViewChecked {
  ...
  timKiemTheos: Array<{ value: any, name: any, checked: boolean }> = [
    ...
    { value: 'MaSoThue', name: 'Mã số thuế', checked: false },
    { value: 'MaKhachHang', name: 'Mã khách hàng', checked: false },
    { value: 'TenKhachHang', name: 'Tên khách hàng', checked: false },
    { value: 'NguoiMuaHang', name: 'Người mua hàng', checked: false },
  ];
  @Input() formData: any;
  ...
  displayData: HoaDonParams = {
    ...
    TrangThaiPhatHanh: 3,
    LoaiHoaDon: 100,//để phân biệt là vào list hay ở tab
  ...
  getData() {
    ...
    console.log('timKiemTheoChecked: ', timKiemTheoChecked);
    ...
    this.hoaDonDienTuService.KiemTraHoaDonDaLapTBaoCoSaiSot(this.dataSelected.hoaDonDienTuId).subscribe((rs: any) => {
      if (rs != null && (!rs.isDaGuiThongBao || !rs.isDaLapThongBao)) {
        if (!rs.isDaLapThongBao) {
          ...
          let cauThongBao = 'Hóa đơn có Ký hiệu <span class = "colorChuYTrongThongBao"><b>' + mauHoaDon + kyHieuHoaDon + '</b></span> Số hóa đơn <span class = "colorChuYTrongThongBao"><b>' + soHoaDon + '</b></span> Ngày hóa đơn <span class = "colorChuYTrongThongBao"><b>' + ngayHoaDon + '</b></span> có sai sót <b>Không phải lập lại hóa đơn</b> nhưng chưa lập <b>Thông báo hóa đơn điện tử có sai sót (Mẫu số 04/SS-HĐĐT)</b>. Bạn cần lập và gửi Thông báo hóa đơn điện tử có sai sót đến CQT trước khi thực hiện thao tác Xóa bỏ hóa đơn. Vui lòng kiểm tra lại!';
        ...
        if (rs.isDaLapThongBao && !rs.isDaGuiThongBao) {
          ...
          let soHoaDon = this.dataSelected.soHoaDon;
          let ngayHoaDon = moment(this.dataSelected.ngayHoaDon).format('DD/MM/YYYY');
          let cauThongBao = 'Hóa đơn có Ký hiệu <span class = "colorChuYTrongThongBao"><b>' + mauHoaDon + kyHieuHoaDon + '</b></span> Số hóa đơn <span class = "colorChuYTrongThongBao"><b>' + soHoaDon + '</b></span> Ngày hóa đơn <span class = "colorChuYTrongThongBao"><b>' + ngayHoaDon + '</b></span> có sai sót <b>Không phải lập lại hóa đơn</b> đã thực hiện lập <b>Thông báo hóa đơn điện tử có sai sót (Mẫu số 04/SS-HĐĐT)</b> nhưng chưa thực hiện gửi CQT. Bạn cần gửi Thông báo hóa đơn điện tử có sai sót đến CQT trước khi thực hiện thao tác Xóa bỏ hóa đơn. Vui lòng kiểm tra lại!';
  ...
  openXoaBo() {
    this.dataSelected.soCTXoaBo = null;
