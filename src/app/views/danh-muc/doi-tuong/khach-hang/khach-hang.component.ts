...
import { DownloadFile, getHeightBangKe, getHeightBangKe4, getHeightBangKeKhongChiTiet, getHeightBangKeKhongChiTiet2, getHeightBangKeKhongChiTiet2_1, getHeightBangKeKhongChiTiet5, getHeightSimpleTable, getListEmptyBangKe, getListEmptyBangKeKhongChiTiet2, getListEmptyBangKeKhongChiTiet3 } from 'src/app/shared/SharedFunction';
...
export class KhachHangComponent extends TabShortKeyEventHandler implements OnInit {
  ...
  mapOfCheckedId: any = {};
  // fix table
  widthConfig = ['150px', '450px', '450px', '150px', '150px', '200px', '150px', '200px', '200px', '150px', '150px'];
  ...
  onResize(event) {
    this.lstBangKeEmpty = getListEmptyBangKe(this.listKhachHang);
  ...
  LoadData(init = false, reset = false, added = null) {
    ...
    this.doiTuongService.GetAllPaging(this.params).subscribe((data: any) => {
      ...
      // delete all
      if (this.listKhachHang.length === 0 && this.params.PageNumber > 1) {
        this.params.PageNumber -= 1;
        this.LoadData();
      }
      this.scrollConfig.y = (getHeightBangKeKhongChiTiet2_1()) + 'px';
      // if (this.listKhachHang && this.listKhachHang.length > 0) {
      //   this.selectedRow(this.listKhachHang[0]);
      // }
      if(added) {
        var itemAdd = this.listKhachHang.find(x=>x.doiTuongId == added.doiTuongId);
        var indexAdd = this.listKhachHang.indexOf(itemAdd);
        if(indexAdd != -1){
          this.selectedRow(this.listKhachHang[indexAdd]);
        }       
        else{
          this.params.PageNumber += 1;
        ...
        if(init && this.listKhachHang.length > 0) this.selectedRow(this.listKhachHang[0]);
        else{
          this.rowScrollerToViewEdit.scrollToRow(this.listKhachHang, "doiTuongId").then((result) => {
  ...
  clickSua(isCopy = false, isView = false) {
    ...
    const vals: any[] = this.listKhachHang.filter(x => x.selected === true);
    ...
    this.doiTuongService.GetById(data.doiTuongId).subscribe(
      (res: any) => {
        const modal = this.ActivedModal = this.modalService.create({
          nzTitle: `${isCopy ? 'Thêm' : 'Sửa'} khách hàng` ,
          nzContent: AddEditKhachHangModalComponent,
          nzMaskClosable: false,
          nzClosable: false,
          nzKeyboard: false,
        ...
        if (rs) {
          this.modalService.create({
            ...
            nzComponentParams: {
              ...
              msCloseText: TextGlobalConstants.TEXT_CONFIRM_CLOSE,
              msTitle: `Kiểm tra lại`,
              msContent: `Mã khách hàng <b class = "colorChuYTrongThongBao">${this.dataSelected.ma}</b> đã có phát sinh trên hóa đơn. Để xóa mã khách hàng này, bạn cần xóa hóa đơn liên quan. Vui lòng kiểm tra lại!`,
              msOnClose: () => {
              },
            }
          });
  ...
  selectedRow(data: any) {
    ...
    if (this.listOfSelected.length === 0) {
      ...
      data.selected = true;
      this.listKhachHang.forEach(element => {
        if (element !== data) {
