...
import { AddEditDonViTinhModalComponent } from '../modals/add-edit-don-vi-tinh-modal/add-edit-don-vi-tinh-modal.component';
...
@Component({
  ...
  styleUrls: ['./don-vi-tinh.component.scss']
})
export class DonViTinhComponent extends TabShortKeyEventHandler implements OnInit {
  ...
  onResize(event) {
    this.lstBangKeEmpty = getListEmptyBangKe(this.listDonViTinh);
  ...
  LoadData(init = false, reset = false, added = null) {
    ...
    this.loading = true;
    this.donViTinhService.GetAllPaging(this.params).subscribe((data: any) => {
      this.listDonViTinh = data.items;
      ...
      // delete all
      if (this.listDonViTinh.length === 0 && this.params.PageNumber > 1) {
        this.params.PageNumber -= 1;
        this.LoadData();
      }
      this.scrollConfig.y = (getHeightBangKeKhongChiTiet2_1()) + 'px';
      // if (this.listDonViTinh && this.listDonViTinh.length > 0) {
      //   this.selectedRow(this.listDonViTinh[0]);
      // }
      if(added) {
        var itemAdd = this.listDonViTinh.find(x=>x.donViTinhId == added.donViTinhId);
        var indexAdd = this.listDonViTinh.indexOf(itemAdd);
        console.log(indexAdd);
        this.selectedRow(this.listDonViTinh[indexAdd]);
      }
      else{
        if(init) {
          if(this.listDonViTinh.length > 0) this.selectedRow(this.listDonViTinh[0])
        }
        else{
          this.rowScrollerToViewEdit.scrollToRow(this.listDonViTinh, "donViTinhId").then((result) => {
  ...
  clickSua(isCopy = false, isView = false) {
    ...
    const vals: any[] = this.listDonViTinh.filter(x => x.selected === true);
    if (vals == null || vals.length < 1) {
      return;
    }

    const data: any = vals[0];
    this.rowScrollerToViewEdit.getRowToViewEdit(data.donViTinhId);
    // call modal
    this.donViTinhService.GetById(data.donViTinhId).subscribe(
      (res: any) => {
        const modal = this.ActivedModal = this.modalService.create({
          nzTitle: `${isCopy ? 'Thêm' : 'Sửa'} đơn vị tính`,
          nzContent: AddEditDonViTinhModalComponent,
          nzMaskClosable: false,
          nzClosable: false,
          nzKeyboard: false,
  ...
  deleteCheckedItem(id: any) {
    this.listOfSelected = this.listOfSelected.filter(x => x.donViTinhId !== id);
  ...
  clickXoa() {
    ...
    if (this.dataSelected) {
      ...
      this.donViTinhService.CheckPhatSinh(this.dataSelected.donViTinhId).subscribe((rs: any) => {
        if (rs) {
          this.modalService.create({
            nzContent: MessageBoxModalComponent,
            nzMaskClosable: false,
            ...
            nzComponentParams: {
              ...
              msCloseText: TextGlobalConstants.TEXT_CONFIRM_CLOSE,
              msTitle: `Kiểm tra lại`,
              msContent: `Đơn vị tính <b class = "colorChuYTrongThongBao">${this.dataSelected.ten}</b> đã có phát sinh trên hóa đơn. Để xóa đơn vị tính này, bạn cần xóa danh mục và hóa đơn liên quan. Vui lòng kiểm tra lại!`,
              msOnClose: () => {
              },
            }
          });
          ...
          const modal = this.ActivedModal =
            this.modalService.create({
              ...
              nzComponentParams: {
                ...
                msCloseText: TextGlobalConstants.TEXT_CONFIRM_CLOSE,
                msOnOk: () => {
                  this.donViTinhService.Delete(this.dataSelected.donViTinhId).subscribe((rs: any) => {
                    if (rs) {
                      this.nhatKyTruyCapService.Insert({
                        loaiHanhDong: LoaiHanhDong.Xoa,
                        refType: RefType.DonViTinh,
                        thamChieu: 'ĐVT: ' + this.dataSelected.ten,
                        refId: this.dataSelected.donViTinhId
                      }).subscribe();
                      this.deleteCheckedItem(this.dataSelected.donViTinhId);
  ...
  selectedRow(data: any) {
    ...
    if (this.listOfSelected.length === 0) {
      ...
      data.selected = true;
      this.listDonViTinh.forEach(element => {
        if (element !== data) {
  ...
  exportExcel() {
    const id = this.message.loading('Loading...', { nzDuration: 0 }).messageId;
    this.donViTinhService.ExportExcel(this.params)
      .subscribe((res: any) => {
        const link = window.URL.createObjectURL(res);
        DownloadFile(link, `Bang_ke_don_vi_tinh.xlsx`);
        this.message.remove(id);
      });
  }
  ...
  // // Checkbo
  // refreshStatus(): void {
  //   this.isAllDisplayDataChecked = this.listOfDisplayData.every(item => this.mapOfCheckedId[item.donViTinhId]);
  //   this.isIndeterminate =
  //     this.listOfDisplayData.some(item => this.mapOfCheckedId[item.donViTinhId]) && !this.isAllDisplayDataChecked;

  //   this.dataSelected = null;
  //   this.listDonViTinh.forEach((item: any) => {
  //     item.selected = false;
  //   });

  //   let entries = Object.entries(this.mapOfCheckedId);
  //   for (const [prop, val] of entries) {
  //     const item = this.listOfDisplayData.find(x => x.donViTinhId === prop);
  //     const selectedIndex = this.listOfSelected.findIndex(x => x.donViTinhId === prop);
  //     const index = this.listDonViTinh.findIndex(x => x.donViTinhId === prop);
  ...
  //     } else {
  //       if (selectedIndex !== -1) {
  //         this.listOfSelected = this.listOfSelected.filter(x => x.donViTinhId !== prop);
  //       }
  //     }

  //     if (index !== -1) {
  //       this.listDonViTinh[index].selected = val;
  //     }
  //   }
  // }

  // checkAll(value: boolean): void {
  //   this.listOfDisplayData.forEach(item => (this.mapOfCheckedId[item.donViTinhId] = value));
  //   this.refreshStatus();
  // }
