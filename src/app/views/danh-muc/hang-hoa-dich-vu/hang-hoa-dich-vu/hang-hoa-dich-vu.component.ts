...
import { AddEditHangHoaDichVuModalComponent } from '../modals/add-edit-hang-hoa-dich-vu-modal/add-edit-hang-hoa-dich-vu-modal.component';
...
@Component({
  selector: 'app-hang-hoa-dich-vu',
  templateUrl: './hang-hoa-dich-vu.component.html',
  styleUrls: ['./hang-hoa-dich-vu.component.scss']
})
export class HangHoaDichVuComponent extends TabShortKeyEventHandler implements OnInit {
  ...
  mapOfCheckedId: any = {};
  // fix table
  widthConfig = ['150px', '200px', '150px', '200px', '150px', '150px', '150px'];
  ...
  onResize(event) {
    this.lstBangKeEmpty = getListEmptyBangKe(this.listHHDV);
  ...
  LoadData(init = false, reset = false, added = null) {
    ...
    this.loading = true;
    this.hangHoaDichVuService.GetAllPaging(this.params).subscribe((data: any) => {
      this.listHHDV = data.items;
      ...
      // delete all
      if (this.listHHDV.length === 0 && this.params.PageNumber > 1) {
        this.params.PageNumber -= 1;
        this.LoadData();
      }
      // this.refreshStatus();
      this.scrollConfig.y = (getHeightBangKeKhongChiTiet2_1()) + 'px';
      // if (this.listHHDV && this.listHHDV.length > 0) {
      //   this.selectedRow(this.listHHDV[0]);
      // }
      if(init){ 
        if(this.listHHDV.length > 0)
        this.selectedRow(this.listHHDV[0]);
      }
      if(added) {
        var itemAdd = this.listHHDV.find(x=>x.hangHoaDichVuId == added.hangHoaDichVuId);
        var indexAdd = this.listHHDV.indexOf(itemAdd);
        console.log(indexAdd);
        if(indexAdd != -1){
          this.selectedRow(this.listHHDV[indexAdd]);
        }       
        else{
          this.params.PageNumber += 1;
        ...
        this.rowScrollerToViewEdit.scrollToRow(this.listHHDV, "hangHoaDichVuId").then((result) => {
  ...
  clickSua(isCopy = false, isView = false) {
    ...
    const vals: any[] = this.listHHDV.filter(x => x.selected === true);
    if (vals == null || vals.length < 1) {
      return;
    }

    const data: any = vals[0];
    this.rowScrollerToViewEdit.getRowToViewEdit(data.hangHoaDichVuId);
    // call modal
    this.hangHoaDichVuService.GetById(data.hangHoaDichVuId).subscribe(
      (res: any) => {
        const modal = this.ActivedModal = this.modalService.create({
          nzTitle: `${isCopy ? 'Thêm' : 'Sửa'} hàng hóa, dịch vụ`,
          nzContent: AddEditHangHoaDichVuModalComponent,
          nzMaskClosable: false,
          nzClosable: false,
          nzKeyboard: false,
  ...
  deleteCheckedItem(id: any) {
    this.listOfSelected = this.listOfSelected.filter(x => x.hangHoaDichVuId !== id);
  ...
  clickXoa() {
    ...
    if (this.dataSelected) {
      this.hangHoaDichVuService.CheckPhatSinh(this.dataSelected.hangHoaDichVuId).subscribe((rs: any) => {
        if (rs) {
          this.modalService.create({
            nzContent: MessageBoxModalComponent,
            nzMaskClosable: false,
            ...
            nzComponentParams: {
              ...
              msCloseText: TextGlobalConstants.TEXT_CONFIRM_CLOSE,
              msTitle: `Kiểm tra lại`,
              msContent: 'Mã hàng hóa, dịch vụ <b class = "colorChuYTrongThongBao">' + this.dataSelected.ma + '</b> đã có phát sinh trên hóa đơn. Để xóa mã hàng hóa, dịch vụ này, bạn cần xóa hóa đơn liên quan. Vui lòng kiểm tra lại!',
              msOnClose: () => {
              },
            }
          });
          ...
          const modal = this.ActivedModal =
            this.modalService.create({
              ...
              nzComponentParams: {
                ...
                msCloseText: TextGlobalConstants.TEXT_CONFIRM_CLOSE,
                msOnOk: () => {
                  this.hangHoaDichVuService.Delete(this.dataSelected.hangHoaDichVuId).subscribe((rs: any) => {
                    if (rs) {
                      this.nhatKyTruyCapService.Insert({
                        ...
                        refId: this.dataSelected.hangHoaDichVuId
                      }).subscribe();
                      this.deleteCheckedItem(this.dataSelected.hangHoaDichVuId);
  ...
  exportExcel() {
    const id = this.message.loading('Loading...', { nzDuration: 0 }).messageId;
    this.hangHoaDichVuService.ExportExcel(this.params)
      .subscribe((res: any) => {
        const link = window.URL.createObjectURL(res);
        DownloadFile(link, `Bang_ke_hang_hoa_dich_vu.xlsx`);
        this.message.remove(id);
      });
  }
